...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        Cursor cursor=null;
        try{
            cursor = db.query(TodoContract.TodoEntry.TABLE_NAME,new String[]{TodoContract.TodoEntry.COLUMN_ID,TodoContract.TodoEntry.COLUMN_CONTENT,TodoContract.TodoEntry.COLUMN_DATE,TodoContract.TodoEntry.COLUMN_STATE,TodoContract.TodoEntry.COLUMN_IMPORTANT},
                    null,null,null,null,TodoContract.TodoEntry.COLUMN_IMPORTANT+" DESC, "+TodoContract.TodoEntry.COLUMN_DATE+" DESC");

            while(cursor.moveToNext())
            {
                int intid = cursor.getInt(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_ID));
                ...
                int intimpo = cursor.getInt(cursor.getColumnIndex(TodoContract.TodoEntry.COLUMN_IMPORTANT));

                Note note = new Note(intid);
                note.setContent(content);
                note.setDate(new Date(dateMs));
                note.setState(State.from(intstate));
                note.setImportant(intimpo);
    ...
    private void deleteNote(Note note) {
        // TODO 删除数据
        String selection = TodoContract.TodoEntry.COLUMN_ID + " LIKE ?";
    ...
    private void updateNode(Note note) {
        ...
        values.put(TodoContract.TodoEntry.COLUMN_STATE,note.getState()==State.DONE?1:0);
